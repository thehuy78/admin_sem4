...
import { status, headerjson, ListTestingPage, fee } from "../data/DataTesting"
  ...
  const fetchdata = useCallback(async () => {
    try {
      if (token) {
        ...
        setIsLoading(true)
        var rs = await apiRequestAutherize("post", "testing/getbyhospital", token, f)
        console.log(rs.data);
        if (rs && rs.data && rs.data.status === 200) {
          setTesting(rs.data.data.content)
    ...
    <ListTestingPage>
      ...
      {testing && testing.length > 0 ? (
        <section className='brums_nav'>
          ...
          <span>{testing[0].hospitalName}</span>
          <i className="fa-solid fa-angle-right"></i>
              ...
              {testing &&
                ...
                testing.map((item, index) => (
                  <tr key={index}>
                    ...
                    <td>
                      <Link to={`/admin/booking/testing/${item.code}`} className='link_tag'>
